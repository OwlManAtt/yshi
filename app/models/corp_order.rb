class CorpOrder < ActiveRecord::Base
  ...
  def self.find_latest_sell_orders
    find(:all, :order => 'issued DESC', :conditions => {:order_type => 'sell'}, :limit => 7)
  end
  ...
  def self.activity_summary
    ["SELECT 
      ...
      "SELECT 
        'Ore Mining' AS activity,
        SUM((unit_material_cost * volume_entered)) AS income
      FROM corp_orders 
      INNER JOIN item_types ON corp_orders.item_type_id = item_types.id 
      WHERE corp_orders.order_type = 'sell'
      ...
      "SELECT 
        ...
        SUM(((price - unit_material_cost) * volume_entered)) AS income
      FROM corp_orders 
      INNER JOIN item_types ON corp_orders.item_type_id = item_types.id 
      WHERE corp_orders.order_type = 'sell'
      AND item_types.item_group_id NOT IN (423)
  ...
  def margin
    ...
    100 * profit_per_unit / unit_material_cost
  end
end # CorpOrder
